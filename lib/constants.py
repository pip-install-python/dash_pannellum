...
# `curl /healthz`, a smoke battery, a link audit, the container's own
# HEALTHCHECK. It is not the same shape as `internal_ua()`, which names this
# app calling a peer server-to-server, and the difference is worth a spelling
# of its own so a reader of the far side's logs can tell "a host checking
# itself" from "a host using another host".
#
...
#     UA: that string classifies `bot_type='monitor'` off the word
#     "monitoring" alone, so a probe named that way changes the document it
#     was sent to measure.
#
...
# time — and on this host `record_read` only learned that rule at SYNC-1.6.43
# item 1, which is why the pre-09-02 vendor rows in this repo's ledger are
# the network's own probes and are not to be trusted.
#
# Lane, vendor and class hold BY CONSTRUCTION, and
# `tests/test_internal_traffic.py` RE-MEASURES that against the installed
# package rather than trusting this comment — a floor bump is exactly what
# would move it.
...
def probe_ua(engine: str, caller: str = "") -> str:
    ...
    ``2plot-monitoring`` measurement above).
    """
